...
class BenchmarkService:
    ...
    def _get_llms_with_billing(self, company_id: str, agent_id: str):
        ...
        # 🆕 LLM JUDGE - Claude Sonnet 4.5 para avaliar qualidade dos chunks
    ...
    def run_company_benchmark(
        ...
        Args:
            ...
            sample_size: Número de documentos para testar (default 2)
        ...
        logger.info(
            f"🏢 Agent: {agent_id} | Sample: {sample_size} docs | Questions: {total_questions} | Threshold: {threshold}"
        )
        ...
        try:
            ...
            logger.info("[Benchmark] 🧑‍⚖️ LLM Judge (Claude Sonnet 4.5) initialized")

            # === FASE A: Geração do Dataset ===
            logger.info("\n📚 [PHASE A] Generating Dataset...")
            self.update_job_status(job_id, 5, "generating_dataset")

            # 1. Selecionar documentos (🔥 COM FILTRO POR AGENT_ID)
            docs_query = (
                self.supabase.table("documents")
                .select("id, file_name, company_id, agent_id")
            ...
            available_docs = docs_query.data
            if len(available_docs) < sample_size:
                raise ValueError(
                    f"Agent {agent_id} needs at least {sample_size} completed documents, found {len(available_docs)}."
                )

            sample_docs = random.sample(available_docs, sample_size)
            ...
            questions_per_doc = max(1, total_questions // sample_size)
            remaining = total_questions - (questions_per_doc * sample_size)
