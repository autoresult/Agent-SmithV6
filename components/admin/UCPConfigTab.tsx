  ...
  return (
    <div className="space-y-6 mt-6">
      ...
      <Card className="bg-green-900/20 border-green-700">
        <CardContent className="p-4">
          <div className="flex items-start gap-3">
            <ShoppingBag className="h-5 w-5 text-green-400 mt-0.5" />
            <div>
              ...
              <p className="text-xs text-green-300 mt-1">
                ...
                <code className="bg-green-800/50 px-1 rounded">/.well-known/ucp</code>
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <Card className="bg-[#2D2D2D] border-[#3D3D3D]">
          ...
          <CardContent className="space-y-3">
            {connections.map((conn) => (
              <div
                key={conn.id}
                className="flex items-center justify-between p-4 bg-green-900/20 border border-green-700 rounded-lg"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-xs text-gray-400">v{conn.manifest_version}</span>
      ...
      <Card className="bg-[#2D2D2D] border-[#3D3D3D]">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs text-gray-400">URL da Loja</label>
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="outline"
                className="border-blue-600 text-blue-400 hover:bg-blue-600/20 whitespace-nowrap"
              >
                {discovering ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
            ...
            <p className="text-xs text-gray-500">
              ...
              <code className="bg-[#1A1A1A] px-1 rounded">/.well-known/ucp</code> automaticamente.
            </p>
          </div>
            ...
            >
              {discoveryResult.success ? (
                <>
                  ...
                  <div className="mb-3">
                    <p className="text-xs text-gray-400 mb-2">Capabilities disponíveis:</p>
                    <div className="flex flex-wrap gap-1">
                      {discoveryResult.capabilities.map((cap) => (
                        <Badge key={cap.name} variant="secondary" className="text-xs bg-[#1A1A1A]">
                          {cap.tool_name}
                        </Badge>
                      ))}
                    </div>
      ...
      {connections.length > 0 && (
        <Card className="bg-[#2D2D2D] border-[#3D3D3D]">
          <CardHeader>
            <CardTitle className="text-sm text-white">Variáveis para Prompt</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-gray-400 mb-3">Use estas variáveis no prompt do agente:</p>
              ...
              {connections.flatMap((conn) =>
                conn.capabilities.map((cap) => {
                  ...
                  return (
                    <div
                      key={`${conn.id}-${cap}`}
                      className="flex items-center justify-between p-3 bg-[#1A1A1A] rounded border border-[#3D3D3D]"
                    >
                      <div>
                        <code className="text-green-400 text-sm font-mono">
                          {`{{ucp_${toolName}}}`}
                        </code>
                        <p className="text-xs text-gray-500 mt-1">{cap}</p>
                      </div>
                      <Badge variant="outline" className="text-xs border-gray-600">
              ...
              {connections.map((conn) => (
                <React.Fragment key={conn.id}>
                  <div className="flex items-center justify-between p-3 bg-blue-900/20 rounded border border-blue-700">
                    ...
                    <Badge variant="outline" className="text-xs border-blue-600">
                      {conn.store_url.replace('https://', '')}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between p-3 bg-purple-900/20 rounded border border-purple-700">
                    <div>
                    ...
                    <Badge variant="outline" className="text-xs border-purple-600">
                      {conn.store_url.replace('https://', '')}
                    </Badge>
                  </div>
                </React.Fragment>
