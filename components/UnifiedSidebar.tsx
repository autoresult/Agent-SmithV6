...
export function UnifiedSidebar({
  ...
  const loadConversations = async () => {
    try {
      const response = await fetch('/api/conversations?limit=15');
      if (response.ok) {
        const data = await response.json();
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-[#2f2f2f] border-r border-white/10 transform transition-transform duration-200 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        <div className="flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 px-2 mb-6">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <div className="flex flex-col mt-1">
                  <span className="text-xs text-gray-300 truncate">{userProfile.name}</span>
          ...
          <div className="flex-1 overflow-y-auto">
            {isOnChatPage && conversations.length > 0 && (
              <div className="p-4 border-b border-white/30">
                <h3 className="text-xs uppercase text-white/50 mb-2 font-semibold">
                  Conversas Recentes
                </h3>
                  ...
                  {conversations.map((conv) => (
                    ...
                    >
                      <div className="flex items-start gap-2">
                        <MessageSquare
                          size={16}
                          className={`mt-1 flex-shrink-0 ${conv.session_id === currentSessionId ? 'text-blue-400' : 'text-white/30'}`}
                        />
                        <div className="flex-1 min-w-0">
                          {/* LINHA DE TÍTULO + DATA */}
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <p
                              className={`text-sm font-medium truncate flex-1 ${conv.session_id === currentSessionId ? 'text-white' : 'text-white/70'}`}
                            >
                          ...
                          {conv.agents?.name && (
                            <div className="flex">
                              <Badge
                                variant="outline"
                                className="text-[9px] px-1.5 py-0 h-4 border-white/10 bg-white/5 text-emerald-400 font-normal rounded-sm"
                              >
                                {conv.agents.name}
                              </Badge>
                            </div>
            ...
            <nav className="p-4">
              <h3 className="text-xs uppercase text-white/50 mb-2 font-semibold">Menu</h3>
              <div className="space-y-1">
                {navItems.map((item) => (
                  <Link
                    key={item.href}
          ...
          <div className="p-4 border-t border-white/30 space-y-3">
            ...
            <div className="bg-blue-900/20 border border-blue-800/30 rounded-lg p-3">
              <p className="text-xs text-gray-400 font-medium mb-1">Agent Smith v6.0</p>
