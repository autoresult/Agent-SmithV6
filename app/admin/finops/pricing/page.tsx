  ...
  return (
    <div className="p-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">💰 Tabela de Custos LLM</h1>
          <p className="text-gray-400">Preços por milhão de tokens (input/output)</p>
        </div>
      ...
      <div className="flex gap-4 mb-6">
        ...
        <Select value={filterProvider} onValueChange={setFilterProvider}>
          <SelectTrigger className="w-[180px] bg-[#1A1A1A] border-[#3D3D3D] text-gray-300">
          ...
          <SelectContent className="bg-[#1A1A1A] border-[#3D3D3D]">
            ...
            <SelectItem value="anthropic" className="text-gray-300">
              🟠 Anthropic
            </SelectItem>
            <SelectItem value="openai" className="text-gray-300">
              🟢 OpenAI
            </SelectItem>
            <SelectItem value="google" className="text-gray-300">
              🔵 Google
            </SelectItem>
            <SelectItem value="other" className="text-gray-300">
              ⚪ Outros
            </SelectItem>
          </SelectContent>
        </Select>
            ...
            <tbody className="divide-y divide-[#2D2D2D]">
              {Object.entries(groupedPricing).map(([provider, items]) => (
                <Fragment key={provider}>
                  ...
                  {/* Provider Items */}
                  {items.map((item) => (
                    <tr key={item.id} className="hover:bg-[#2D2D2D]/50">
                      ...
                      <td className="px-6 py-4 text-right">
                        {editingId === item.id ? (
                          <Input
                            ...
                            className="w-24 bg-[#0A0A0A] border-blue-500 text-white text-right"
                          />
                        ) : (
                          <span
                            className={`text-white ${item.unit === 'minute' ? 'text-blue-400' : ''}`}
                          >
                      ...
                      <td className="px-6 py-4 text-right">
                        {editingId === item.id ? (
                          <Input
                            ...
                            className="w-20 bg-[#0A0A0A] border-blue-500 text-white text-right"
                          />
                        ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        {editingId === item.id ? (
                          ...
                          <span
                            className={`px-2 py-1 rounded text-xs font-medium ${item.is_active ? 'bg-blue-600/20 text-blue-400' : 'bg-zinc-700 text-zinc-400'}`}
                          >
