...
export default function AdminAllUsersPage() {
  ...
  const getRoleBadge = (user: SafeUser) => {
    ...
    if (
      ...
      return (
        <Badge className="bg-yellow-500/20 text-yellow-400 border-yellow-500/30">👑 Owner</Badge>
      );
    }
    ...
    if (user.role === 'admin_company' || user.role === 'owner' || user.role === 'admin') {
      return (
        <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/30">🛡️ Admin</Badge>
      );
    }

    // Member por padrão
    return <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30">👤 Membro</Badge>;
  };
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Todos os Usuários</h1>
        <p className="text-gray-400">Visualize e gerencie todos os usuários do sistema</p>
      </div>
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          <SelectTrigger className="w-full sm:w-[200px] bg-[#2D2D2D] border-[#3D3D3D] text-white">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {/* Pagination Controls */}
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-4 bg-[#1A1A1A] border border-[#2D2D2D] rounded-lg">
